...
export function LandingPage() {
  const [waitlistStats, setWaitlistStats] = useState<{ count: number; target: number } | null>(
    null,
  );
          ...
          <GoalCarousel count={GOALS.length}>
            {GOALS.map((goal) => {
              ...
              return (
                ...
                >
                  <Link href={`/intake?goal=${goal.id}`} className="group block h-full">
        ...
        <div className="relative mx-auto max-w-6xl px-4 py-16 text-center sm:py-20">
          <Reveal>
            ...
            <p className="mx-auto mt-4 max-w-xl text-muted-foreground">
              Strength, powerbuilding, or meet prep — coach-reviewed and delivered in{" "}
              {SLA_HOURS} hours.
            </p>
            <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
              <Link
                href="/intake"
                className={cn(buttonVariants({ size: "lg" }), "h-12 gap-2 px-8 text-base shadow-lg shadow-primary/20")}
              >
