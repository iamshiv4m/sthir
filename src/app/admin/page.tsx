...
type QueueItem = {
  ...
  program?: { id: string; templateName: string; coachNotes: string; blocks: unknown[] };
};
...
export default function AdminPage() {
  ...
  async function reviewAction(intakeId: string, action: "approve" | "deliver" | "reject") {
    ...
    await fetch(apiUrl(`/admin/programs/${intakeId}`), {
      method: "POST",
      headers,
      body: JSON.stringify({ action, coachNotes: notes, reviewerId: "founder" }),
    });
    setSelected(null);
    setNotes("");
    load();
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-12">
      ...
      <p className="mt-1 text-muted-foreground">12-hour SLA queue — approve before delivery</p>
      ...
      <div className="mt-8 grid gap-8 lg:grid-cols-2">
        <div className="space-y-3">
          {queue.length === 0 && (
            <p className="text-muted-foreground">No items in queue. Submit via /intake</p>
          )}
          {queue.map((item) => (
            <button
              key={item.id}
              ...
              className={cn(
                ...
                item.urgent ? "border-destructive/50 bg-destructive/5" : "border-border",
            ...
            >
              ...
              <p className="text-sm text-muted-foreground">{item.answers.email}</p>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex flex-wrap gap-2">
                <Button variant="secondary" onClick={() => reviewAction(selected.id, "approve")}>
                  Approve
                </Button>
                <Button onClick={() => reviewAction(selected.id, "deliver")}>
                  Deliver (CSV/PDF)
                </Button>
                <Button variant="destructive" onClick={() => reviewAction(selected.id, "reject")}>
                  Reject
                </Button>
              </div>
              {selected.program?.id && (
